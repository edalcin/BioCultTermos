...
| **Campos gerenciados** | `comunidades.tipo`, `comunidades.plantas.nomeVernacular`, `comunidades.plantas.tipoUso`, `comunidades.atividadesEconomicas` — mais um vocabulário estático de referência de tipos de uso (`src/data/referenceTerms.js`, ~450 termos do domínio etnobotânico), semeado a cada ciclo de aquisição independente do que já foi digitado em algum registro. Nomenclatura científica **não é** vocabulário controlado (ADR-014): `comunidades.plantas.nomeCientifico` saiu do escopo em 2026-08-10 e os 864 conceitos derivados foram removidos do `etnotermos` — o nome científico permanece dado da Evidência no BioCultDB, sob autoridade externa (ICN/ICZN, WFO/IPNI/GBIF). Ver `BioCultDB/docs/curadoria/decisao-nomes-cientificos-fora-de-escopo.md` |
