...
| **Campos gerenciados** | `comunidades.tipo`, `comunidades.plantas.nomeVernacular`, `comunidades.plantas.tipoUso`, `comunidades.atividadesEconomicas` — mais um vocabulário estático de referência de tipos de uso (`src/data/referenceTerms.js`, ~450 termos do domínio etnobotânico), semeado a cada ciclo de aquisição independente do que já foi digitado em algum registro. `comunidades.plantas.nomeCientifico` saiu do escopo de curadoria em 2026-08-10; os ~864 conceitos já semeados permanecem preservados e congelados — ver `BioCultDB/docs/curadoria/decisao-nomes-cientificos-fora-de-escopo.md` |
